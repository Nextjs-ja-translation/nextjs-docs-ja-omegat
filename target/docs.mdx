...
- 完全自動の (webpack と babel を利用した) トランスパイルとビルド
- コードのホットリローディング
- `./pages` 配下のファイルの構造化とサーバーサイドレンダリング
- 静的ファイルの配布`./static/` は `/static/` にマッピングされます (プロジェクトに [`./static/` ディレクトリを作成](#static-file-serving-eg-images)する必要があります。)

実際にこれがどれだけシンプルにできるかは、[nextgram というサンプルアプリケーション](https://github.com/zeit/nextgram)をチェックしてみるとよいでしょう。
...
既存のどんな CSS-in-JS も利用することができます。一番シンプルなのはインラインで記述するものでしょう:

```jsx
export default () => <p style={{ color: 'red' }}>hi there</p>
```

より洗練された CSS-in-JS のソリューションを使いたい場合には、サーバーサイドレンダリングでスタイルを設定するための実装を追加する必要があるでしょう。これは、独自の[カスタム `<Document>`](#user-content-custom-document) コンポーネントを定義することで実現できます。
...
プロジェクトのルートディレクトリに `static` というディレクトリを作成します。From your code you can then reference those files with `/static/` URLs:

```jsx
export default () => <img src="/static/my-image.png" alt="my image" />
```

_注意: ディレクトリ名は必ず `static` としてください。ディレクトリはこの名前である必要があり、Next.js が静的なアセットをサーブするための唯一のディレクトリとなります。_
...
_注意: `<head>` の内容はコンポーネントがアンマウントされる際に消去されるため、各ページは `<head>` で必要とするものをすべて定義しておく必要があります。他のページで追加しているものに依存することはできません。_
...
状態、ライフサイクルフック、そして **初期データの設定** を行う必要がある場合、(これまで紹介してきたステートレスな関数ではなく) `React.Component` を export することができます:
...
_注意: `getInitialProps` を子コンポーネントで利用することは**できません**。利用できるのは `pages` に限られます。_

<br/>

> もし `getInitialProps` の中でサーバーのみのモジュールを使っている場合、それらを[正しく import](https://arunoda.me/blog/ssr-and-server-only-modules) していないと
> アプリケーションが遅くなってしまうため注意してください。

<br/>

ステートレスなコンポーネントでも `getInitialProps` ライフサイクルメソッドを定義することができます:
...
- `err` - エラーオブジェクト (もしレンダリング中にエラーが発生した場合)
...
クライアントサイドでのルートの遷移は `<Link>` コンポーネントによって実現されます。以下の2つのページがある場合を考えてみてください:
...
__注意: 最大限にパフォーマンスを発揮したければ [`<Link prefetch>`](#prefetching-pages) の利用を検討してください。これによって、リンクとバックグラウンドでのプリフェッチを同時に同時に実現することが可能になります__

クライアントサイドでのルーティングはブラウザとほぼ同じです:

1. コンポーネントを取得する
2. もし `getInitialProps` が定義されていればデータを取得する。エラーが発生した場合は`_error.js` がレンダリングされる
3. 1 と 2 が完了したら `pushState` が実行されて新しいコンポーネントがレンダリングされる

コンポーネントの内部で `pathname` / `query` / `asPath` を弄りたいときは [withRouter](#using-a-higher-order-component) を使ってください。
...
<details>
  <summary><b>例</b></summary>
  <ul>
    <li><a target="_blank" href="/examples/custom-server">基本的なカスタムサーバー</a></li>
    <li><a target="_blank" href="/examples/custom-server-express">Expressとの統合</a></li>
    <li><a target="_blank" href="/examples/custom-server-hapi">Hapiとの統合</a></li>
    <li><a target="_blank" href="/examples/custom-server-koa">Koaとの統合</a></li>
    <li><a target="_blank" href="/examples/parameterized-routing">パラメータ化されたルーティング</a></li>
    <li><a target="_blank" href="/examples/ssr-caching">SSRキャッシング</a></li>
  </ul>
</details>
...
`next` API はこのようになっております:
- `next(opts: object)`

サポートされているオプション一覧:
- `dev` (`bool`) Next.jsをdevモードで 立ち上げるか否か　- デフォルトでは `false`
- `dir` (`string`) Next.jsのプロジェクトがどこに位置しているのか - デフォルトでは `'.'`
- `quiet` (`bool`) サーバー情報を含むエラーメッセージを隠す - デフォルトでは `false`
- `conf` (`object`)  `next.config.js`で使う予定の同じオブジェクト - デフォルトでは `{}`

それから、`start`というscriptを `NODE_ENV=production node server.js`に変更してください。

#### ファイルシステムに基づいたルーティングを無効にする
デフォルトでは, `Next`は`/pages`配下のファイル名とパス名が連動するようになっています。(例えば、`site.com/some-file`にアクセスすると`/pages/some-file.js`が読み込まれます。)
...
`useFileSystemPublicRoutes` は単純にSSRでのファイル名に基づくルーティングを無効にしますが、クライアントサイドでのルーティングは引き続きこれらのパスにアクセス可能です。もしこのオプションを使用するのなら、 あなたはプログラムによって不必要なルートへの誘導に対するガードをするべきです。

クライアントサイドでのファイル名に基づくルーティングにリダイレクトすることを禁じるようにクライアントサイドのルーターを設定したいと思うでしょう。その時は [Intercepting `popstate`](#intercepting-popstate)を参照してください。

#### 動的なアセットプレフィックス

々、私たちは動的に`assetPrefix`を設定する必要があります。これは入ってくるリクエストに基づく`assetPrefix`を変更するときに役に立ちます。
...
<details>
  <summary><b>例</b></summary>
  <ul>
    <li><a target="_blank" href="/examples/with-app-layout">レイアウトとして`_app.js`を使用する</a></li>
    <li><a target="_blank" href="/examples/with-componentdidcatch">`componentDidCatch`をオーバーライドするために`_app.js`を使用する</a></li>
  </ul>
</details>
...
<details>
  <summary><b>例</b></summary>
  <ul><li><a target="_blank" href="/examples/with-styled-components">Styled componentsのカスタムdocumentでの設定</a></li>
  <li><a target="_blank" href="/examples/with-amp">Google AMP</a></li></ul>
</details>

- サーバーサイドでレンダリングされる
- 最初のサーバーサイドでレンダリングされる文書マークアップの内容を変更する
- CSS-in-JSライブラリのためのサーバーサイドレンダリングの実行。CSS-in-JSライブラリとは[styled-components](./examples/with-styled-components), [glamorous](./examples/with-glamorous)>や[emotion](with-emotion)。そしてNext.jsでデフォルトで使用される[styled-jsx](https://github.com/zeit/styled-jsx)です。

`Next.js`のページでは、文書のマークアップまわりの定義はスキップします。例えば、`<html>`, `<body>`などはインクルードする必要はありません。通常の挙動をオーバーライドするには、`./pages/_document.js`にファイルを作成しなくてはいけません。このファイルでは`Document`クラスを以下のように拡張できます:
...
- `renderPage` (`Function`)は実際のReactのレンダリングロジック（同期的）を実行するコールバックです。Aphroditeの[`renderStatic`](https://github.com/Khan/aphrodite#server-side-rendering)のようなサーバーレンダリングのラッパーをサポートするためには、この関数を装飾することが便利です。

__注意: Reactコンポーネントの外にある`<Main />`は、ブラウザによって初期化されません。アプリケーションロジックをここに追加_しないで_ください。もし、すべてのページで共通のコンポーネ
ント(メニューやツールバーのような)が必要なら、代わりに`App`コンポーネントを使用して下さい。__
...
> もしカスタムエラーページを作ったのならば、`next/error`ではなく、独自の`_error`コンポーネントをインポートしなければいけません。
